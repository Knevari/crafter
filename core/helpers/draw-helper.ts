import type { Sprite } from "../gears/sprite/sprite.types.ts";
...
function drawSprite(
  ...
  const destWidth = sprite.size.x * scale.x;
  const destHeight = sprite.size.y * scale.y;

  const offsetX = -destWidth * sprite.origin.x;
  const offsetY = -destHeight * sprite.origin.y;
